...
public class Exercise1 {
    ...
    public static void main(String[] args) {
        ...
        StreamSources.intNumbersStream().flatMap((id -> StreamSources.userStream().filter(user -> user.getId() == id).map(user -> user.getFirstName()))).forEach(System.out::println);
        //OR
        StreamSources.userStream().filter(user -> StreamSources.intNumbersStream().anyMatch(i -> i == user.getId())).map(user -> user.getFirstName()).forEach(System.out::println);
